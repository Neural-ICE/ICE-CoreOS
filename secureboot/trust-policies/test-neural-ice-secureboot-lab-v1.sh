...
mktree() {  # build a closed-world-complete tree of dummy (unsigned) files, 0755 dirs
  ...
  echo "generation_id=29669232382.1" > "$d/signed-boot-provenance.env"
}

expect_reject() {  # $1=label  $2=substr expected in reason  (env passthrough via caller)
  local label="$1" want="$2"; shift 2
  local out rc
  out="$("$@" "$POL" "$TMP/t" "$U" 2>&1)"; rc=$?
  if [[ $rc -ne 0 ]] && grep -qF -- "$want" <<<"$out"; then
    echo "PASS  $label"
  else
    echo "FAIL  $label (rc=$rc) :: $(tail -1 <<<"$out")"; fails=$((fails+1))
  fi
}
...
mktree "$TMP/t"; install -d -m0755 "$TMP/t/usr/lib/systemd/system"; echo x > "$TMP/t/usr/lib/systemd/system/unexpected.service"
...
expect_reject "missing provenance rejected" "" env

echo "---"; [[ $fails -eq 0 ]] && { echo "ALL NEGATIVE TESTS PASSED"; exit 0; } || { echo "$fails FAILURE(S)"; exit 1; }
