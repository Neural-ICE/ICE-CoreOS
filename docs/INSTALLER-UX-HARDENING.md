# Installer UX hardening — the USB install is the PRIMARY install path

**Decision (2026-07-03):** users flash and install the OS **themselves**. The USB
installer is therefore the main experience, not a recovery path. The boot flakiness
observed during the 2026-07-02/03 install session on the GB10 must be engineered
away, not documented around.
...
## Fixes (build-installer-usb.sh / autoinstall) — REQUIRED before shipping
...
1. **Kill the fallback dance on the installer USB**: remove `\EFI\BOOT\fbaa64.efi`
   ...
   entries; a one-shot installer must not touch the machine's NVRAM.)
...
## Install journey (self-service)

```
download installer → flash USB
...
→ RECOVERY KEY shown + saved on the USB ESP (user writes it down)
→ remove USB, reboot → TUI dashboard
```

- The docs must include per-firmware boot-menu instructions (DGX Spark first),
