...
- **Supersedes**: the earlier license-gated-OS / Zincati-masking approach (that
  gating model is now documented **privately**, out of this open-core repo).
...
- **No telemetry**: no usage/state data sent to Neural ICE. The device only
  contacts, functionally: the licensing service + the registry (image).
...
### Open-core: two layers
- **OS layer** = **public** bootc image (GHCR), free update. Auditable, reusable.
- **Application + gating layer** = **private** (proprietary AI stack + licensing).
  The commercial split, gating mechanics and per-customer economics are documented
  **privately**, outside this open-core repo.
...
- (+) Sovereignty + transparency: auditable public base; the proprietary value sits
  in a separate private layer.
...
1. OS image rebuilt **without a license gate** on the OS: remove the licensing wrapper
   from the OS path, **re-enable `bootc-fetch-apply-updates.timer`** (free native update).
...
5. The gating layer + private registry = **containers**, outside the public image
   (private, later phase).
