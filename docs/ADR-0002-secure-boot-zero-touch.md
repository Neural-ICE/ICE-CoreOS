...
- **Status**: Accepted (Option A) — implementation **to do** (external dependency ~2–4 months)
- **Date**: 2026-06-28
...
**BUSINESS prerequisites (upstream human actions, to be provided by Neural ICE):**
- [ ] **Registered company** (legal entity: commercial register / tax ID) —
      the template now requires it; an unregistered individual/project does not qualify.
- [ ] **Microsoft Partner / sysdev** account (**EV** certificate, tied to the legal entity).
- [ ] **HSM / smartcard / air-gapped vault** for the Neural ICE signing key.
- [ ] **2 named security contacts** with **PGP keys** (verified by encrypted email).

**TECHNICAL prerequisites (producible by the team / this pipeline):**
...
- **Cannot gate the HW validation.** The submission is launched **in parallel** as soon as
  company + HSM + contacts are ready.
...
- (−) External dependency **2–4 months** + company/HSM/EV prerequisites.
...
- See also [[ADR-0001-ota-licence-gate]] (OTA model, which remains unchanged).
