...
# The installer asks the resolve-only avahi directly (avahi-resolve) and pins the answer through a
...
# avahi-resolve talks to avahi on this unix socket (avahi-daemon.socket owns it). Its
...
# The LIVE installer's hosts file, where a resolved mirror name is pinned for
# curl, podman and skopeo (NSS `files`). Never the installed system's.
NEURALICE_LIVE_HOSTS="$(ni_path NEURALICE_LIVE_HOSTS /etc/hosts)"
readonly NEURALICE_LIVE_HOSTS
...
assert_mirror_name_resolves() { # $1=host[:port] -> logs the address, or dies by name
  local host=${1%%:*} attempt answer address kept
  ...
  command -v avahi-resolve >/dev/null 2>&1 \
    || die "${MIRROR_NAME_UNRESOLVABLE}: the installer image has no avahi-resolve to ask the resolve-only avahi-daemon for ${host}; nothing has been written to the target disk"
  for (( attempt = 1; attempt <= MIRROR_MDNS_RESOLVE_ATTEMPTS; attempt++ )); do
    # avahi-resolve asks the daemon over its unix socket -- no NSS module is
    # involved (nss-mdns is not in the EL10 repositories; measured 2026-09-09
    # when the installer image build refused it). Its answer is
    # "<host><TAB><address>". A hung resolver is bounded, not waited on.
    if answer="$(timeout --kill-after=2 "$MIRROR_MDNS_RESOLVE_TIMEOUT_SECONDS" avahi-resolve -4 -n "$host" 2>/dev/null)"; then
      address="$(awk 'NF >= 2 { print $2; exit }' <<<"$answer")"
      if [[ "$address" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ && "$address" != 0.0.0.0 ]]; then
        # PINNED FOR EVERY LATER CONSUMER. curl, podman and skopeo resolve
        # through NSS `files`; one line in the LIVE installer's hosts file (a
        # tmpfs overlay the installed system never sees) carries the answer to
        # them, and it is read back through the very NSS path they take.
        kept="$(grep -v -E "[[:space:]]${host//./\\.}\$" "$NEURALICE_LIVE_HOSTS" 2>/dev/null || true)"
        { [[ -z "$kept" ]] || printf '%s\n' "$kept"; printf '%s %s\n' "$address" "$host"; } > "$NEURALICE_LIVE_HOSTS" \
          || die "${MIRROR_NAME_UNRESOLVABLE}: cannot pin ${host} -> ${address} in ${NEURALICE_LIVE_HOSTS}; nothing has been written to the target disk"
        answer="$(timeout --kill-after=2 "$MIRROR_MDNS_RESOLVE_TIMEOUT_SECONDS" getent ahostsv4 "$host" 2>/dev/null)" \
          || die "${MIRROR_NAME_UNRESOLVABLE}: ${host} was pinned to ${address} but NSS does not return it; nothing has been written to the target disk"
        [[ "$(awk 'NF >= 1 { print $1; exit }' <<<"$answer")" == "$address" ]] \
          || die "${MIRROR_NAME_UNRESOLVABLE}: NSS returns another address than the pinned ${address} for ${host}; nothing has been written to the target disk"
        log "LAN mirror ${host} resolves by mDNS (avahi-resolve via the resolve-only avahi-daemon on the management port) to ${address}, pinned in the live installer's hosts file, on attempt ${attempt}/${MIRROR_MDNS_RESOLVE_ATTEMPTS}"
        return 0
      fi
    fi
...
assert_seed_is_the_preseal_release() { # [$1=seed document root; default = the verified ni-seed root]
  ...
  local manifest_path="$root/release-manifest.json"
...
if [[ "$SEED_SOURCE" == mirror ]]; then
  ...
  # the closure's objects follow after the data volume exists (phase 5).
  seed_from_mirror_preflight "$_seed_partuuid"
elif [[ -n "$SEED_CLOSURE" ]]; then
  [[ -n "$_seed_partuuid" ]] \
...
# logically bound images out of the container it runs in
# (install.rs, `query_bound_images(&state.container_root)`), resolves each
# through that container's default containers-storage (boundimage.rs,
# `containers-storage:<Image=>`) and, once the deployment is written, copies
# each one into the deployment's own store with `podman image push <ref>
# containers-storage:[overlay@/run/bootc/storage+…]<ref>` (podstorage.rs) --
# from the default store, which inside this container is the storage
# configuration exported below: the writable tmpfs graph plus the sealed
# medium store as a read-only additional store. (3) Until 2026-09-17 that
# directory was MASKED with an empty one, the deployment received nothing, and
# every first boot re-fetched the same 22 images from the seed store: 14
# minutes of `skopeo copy` per reinstall on the lab GX10 (measured
# 2026-09-16). The medium now carries them in its sealed store, staged by
# digest (image/build-installer-root.sh §1b/§2), so bootc's copy is a local
# one and the appliance boots with them in /usr/lib/bootc/storage.
#
...
# read back through the container's storage at the exact digest its quadlet
# pins, and the source reference is readable — the things phase 4 needs, in
# the order bootc needs them. One bound image missing from the sealed store is
# a refusal here, with the target disk untouched, naming the first one.
# --------------------------------------------------------------------------- #
...
assert_bootc_container_reads_source() { # $1=source imgref bootc will be given
  ...
  podman "${bootc_container_base_args[@]}" --log-driver=none \
    ...
    "$STORE_IMAGE_NAME" sh -c '
      ...
      # Every bound image, as bootc will read it: the bound-images.d of this
      # container, one digest-pinned Image= per quadlet, read back through
      # the storage of this container as the manifest bytes whose SHA-256 is
      # the pinned digest -- the value bootc records when it resolves it.
      n=0; : > /run/ni-probe/bound-refs
      for spec in /usr/lib/bootc/bound-images.d/*.image /usr/lib/bootc/bound-images.d/*.container; do
        case "$spec" in *"*"*) continue ;; esac
        [ -e "$spec" ] || { echo "bound image link ${spec##*/} is dangling; bootc would fail on it after the wipe" > "$r"; exit 1; }
        ...
        want="${ref##*@sha256:}"
        got="$(skopeo inspect --raw "containers-storage:$ref" 2>/run/ni-probe/skopeo.err | sha256sum | cut -c1-64)"
        if [ "$got" != "$want" ]; then
          echo "bound image $ref is not in the sealed store at its pinned digest (store answers ${got:-nothing}: $(head -c 200 /run/ni-probe/skopeo.err | tr "\n" " ")); the medium was cut without it" > "$r"; exit 1
        fi
  ...
  log "bootc container proved before the wipe: fuse-overlayfs lent, ${bound_count} bound images present in the sealed store at their pinned digests (bootc copies them into the deployment), source ${source} readable"
}
...
# store, storage config, lent fuse-overlayfs helper, the image's own bound
# images resolvable in the sealed store).
...
bg_stop
# THE COPY LANDED, READ OFF THE TARGET. bootc's exit status covers its own
# `podman image push` of each bound image, but the appliance boots on what is
# in ostree/bootc/storage (the physical path behind /usr/lib/bootc/storage,
# bootc store/mod.rs BOOTC_ROOT), not on a status. Every reference the
# pre-wipe probe proved present must be named by the deployment's store index
# before this install is allowed to reboot into it: a deployment missing one
# would pull it from the registry at first boot, which is the 14 minutes this
# medium exists to remove -- or, offline, would refuse to start the unit.
...
if (( BOOTC_BOUND_IMAGE_COUNT > 0 )); then
  [[ -f "$_bootc_store_index" ]] \
    || die "bootc reported success but the deployment has no bound image store at ostree/bootc/storage; the ${BOOTC_BOUND_IMAGE_COUNT} bound images were not copied"
  while IFS= read -r _bound_ref; do
    [[ -n "$_bound_ref" ]] || continue
    grep -Fq -- "\"${_bound_ref}\"" "$_bootc_store_index" \
      || die "the deployment's bound image store does not name ${_bound_ref}; bootc did not copy it from the sealed store"
...
_seed_dst=""
if [[ "$SEED_SOURCE" == mirror ]]; then
  # §2d: the six documents were fetched and reconciled before the wipe; every
  # object the closure names is fetched HERE, onto the encrypted volume, each
  ...
  rm -rf -- "$_seed_dst"
  seed_from_mirror_materialize "$_seed_dst"
  _seed_staged=1
elif [[ -n "$SEED_VERIFIED_ROOT" ]]; then
  seed_total=0
...
if (( _seed_staged == 1 )); then
  ...
  # success and leave the appliance to discover it on first boot. For a
  # mirror-sourced seed this is ALSO the first whole-tree proof: §2d proved each
  # document and each object by its own name, and this proves the set.
  ...
  umount /run/seed-dst
  # Only the ni-seed path mounted a seed; the mirror path read from tmpfs.
  [[ -z "$SEED_VERIFIED_ROOT" ]] \
