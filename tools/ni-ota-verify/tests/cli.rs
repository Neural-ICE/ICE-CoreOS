...
//! real key and emits the TCG-marshalled TPMT_SIGNATURE a TPM emits, so the
//! liveness challenge exercises production code end to end.
...
impl Fixture {
    ...
    fn write_access_profile_anchor_raw(&self, profile: &str, seq: u64, defect: AnchorDefect) {
        ...
        // domain separation, the canonical serialisation and the TPMT -> DER
...
\"schema\":\"neural-ice-device-root-tpm-v1\",\"spki_sha256\":\"{spki_sha256}\"}}\n"
    ...
    /// the anchor was signed with and emits the TCG-marshalled `TPMT_SIGNATURE`
    /// a TPM emits. That makes the liveness challenge a real end-to-end exercise
    /// of `tss_signature_to_der` and the verification that follows it.
...
"#,
        ...
        fs::write(
            ...
            format!(
                r#"#!/bin/sh
# The verifier requests the stable TSS form. Sign for real, then marshal the DER
# pair as TPMT_SIGNATURE so the verifier's own strict parser is under test.
out=""
msg=""
format=""
while [ "$#" -gt 0 ]; do
...
der="$(mktemp)" || exit 2
if [ "${{NI_TEST_TPM_WRONG_NONCE:-}}" = 1 ]; then
  printf 'not-the-challenge' > "$der.message" || exit 2
  msg="$der.message"
fi
key='{key}'
if [ "${{NI_TEST_TPM_FOREIGN_KEY:-}}" = 1 ]; then key='{foreign}'; fi
openssl dgst -sha256 -sign "$key" -out "$der" "$msg" || {{ rm -f "$der" "$der.message"; exit 2; }}
...
r, rest = take(body)
s, _ = take(rest)
out = bytes.fromhex("0018000b0020") + r.to_bytes(32, "big")
out += bytes.fromhex("0020") + s.to_bytes(32, "big")
open(sys.argv[2], "wb").write(out)
EOF
case "${{NI_TEST_TPM_SIGNATURE_SHAPE:-}}" in
  malformed) printf '\\000' | dd of="$out" bs=1 seek=1 conv=notrunc 2>/dev/null ;;
  trailing) printf '\\000' >> "$out" ;;
esac
rm -f "$der" "$der.message"
"#,
...
fn delegated_beta_binds_signed_release_receipt_and_immutable_target() {
    ...
    let (code, _, stderr) = command_on_machine(&cfg, "NI_TEST_TPM_WRONG_NONCE", "1");
    assert_eq!(
        code, 1,
        "a signature over another nonce was accepted: {stderr}"
    );
    assert!(
        stderr.contains("did not prove possession of the anchored key"),
        "{stderr}"
    ...
    for shape in ["malformed", "trailing"] {
        let (code, _, stderr) = command_on_machine(&cfg, "NI_TEST_TPM_SIGNATURE_SHAPE", shape);
        assert_eq!(code, 1, "a {shape} TPMT_SIGNATURE was accepted: {stderr}");
        assert!(
            stderr.contains("outside the ECDSA-P256/SHA-256 TPMT_SIGNATURE contract"),
            "{stderr}"
        );
    }
