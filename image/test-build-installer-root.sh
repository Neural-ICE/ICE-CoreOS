...
src=""
for arg in "$@"; do
  case "$arg" in containers-storage:sha256:*) src="${arg#containers-storage:sha256:}" ;; esac
done
dest="${*: -1}"
name="${dest##*]}"
...
grep -Fq "containers-storage:sha256:$IMAGE_ID" "$TMP/store-test/skopeo.args" \
  || fail "the store is not staged FROM the immutable installer image ID"
...
for immutable_step in \
  ...
  'build --type raw --local --config /config.toml "$INSTALLER_IMAGE_REF"'; do
  grep -Fq -e "$immutable_step" "$USB" \
    || fail "a build step does not use the immutable image identity: $immutable_step"
done
