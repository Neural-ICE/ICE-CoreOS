...
# agreed about a lie.
grep -Fq 'create --pull=never --network=none --name "$_candidate_probe"' "$AUTOINSTALL" \
  || fail "the installer does not stage the pulled image in a created container for host-side inspection"
grep -Fq -- '--entrypoint /nonexistent "$_candidate_image_ref"' "$AUTOINSTALL" \
...
grep -Fq 'image mount "$_candidate_image_ref"' "$AUTOINSTALL" \
...
grep -Eq 'run .*--entrypoint .+"\$_candidate_image_ref" cat ' "$AUTOINSTALL" \
